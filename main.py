...
def make_post_requests(url, data):
    ...
    print_info(f'Url: {url}, Status: {response.status_code}', mtype='info')
    return response
...
def print_info(msg, mtype):
    """
    Print a formatted message with timestamp and type using rich.
    mtype: 'info', 'error', 'success', 'warning'
    """
    color_map = {
        'info': 'bold blue',
        'error': 'bold red',
    ...
    color = color_map.get(mtype, 'bold yellow')
    label = mtype.upper() if mtype in color_map else 'WARNING'
    print(f"[{color}]{time_now()} - {label}: {msg}[/{color}]")
...
def get_data(min_dsr, max_dsr,min_renters = 0, max_renters=100 ,state=None, save_info=False):
    ...
    if warnings:
        warning = warnings.get('warning')
        print_info(f"Warnings: {warning}", mtype='warning')
        if save_info:
            print_info(f"Saving warning to info.txt", mtype='info')
    ...
    if not mkts:
        ...
        more = False
        return infos, more
    print_info(f"Markets found: {len(mkts)}. Total data: {COUNT}", mtype='success')
    ...
    if len(mkts) >= 250:
        ...
        print_info(f"Hit 250+ results limit - logging to logs.txt", mtype='warning')
    ...
    print_info(f"Sleeping for {sleep_time:.2f} seconds to avoid hitting the server too hard", mtype='info')
...
def load_cookies_from_json(json_path="cookies.json"):
    ...
    if not os.path.exists(json_path):
        print_info(f"Cookie file '{json_path}' does not exist.", mtype="error")
        return {}
    try:
        with open(json_path, "r") as f:
            cookies_list = json.load(f)
        cookies_dict = {cookie["name"]: cookie["value"] for cookie in cookies_list}
        print_info(f"Loaded cookies from '{json_path}'.", mtype="success")
        return cookies_dict
    except Exception as e:
        print_info(f"Failed to load cookies: {e}", mtype="error")
        return {}
...
def is_logged_in():
    ...
    if email and access_token:
        print_info(f"User is logged in as: {email.text.strip()}", mtype='info')
        print_info(f"Access token: {ACCESS_TOKEN}", mtype='info')
        return True
    else:
        print_info("User is not logged in.", mtype='warning')
        return False
...
def remove_duplicates(filename='markets.csv'):
    if not os.path.exists(filename):
        print_info(f"File {filename} does not exist. No duplicates to remove.", mtype='warning')
        return
    df = pd.read_csv(filename)
    if df.empty:
        print_info(f"File {filename} is empty. No duplicates to remove.", mtype='warning')
        return
    ...
    print_info(f"Removed duplicates from {filename}. Initial count: {initial_count}, Final count: {final_count}", mtype='success')
    print_info(f"Clean data saved to {new_filename}", mtype='success')
...
def main():
    ...
    while True:
        print_info(f"Searching for markets with DSR between {min_dsr} and {max_dsr}", mtype='info')
        ...
        if len(data) >= 250:
            ...
            for state in stat_values:
                print_info(f"Searching for markets with DSR between {min_dsr} and {max_dsr} in {state}", mtype='info')
                state_data, _ = get_data(min_dsr, max_dsr, state=state)
                
                if len(state_data) >= 250:
                    print_info(f"Hit 250+ results limit for DSR {min_dsr}-{max_dsr} in {state}. Segmenting by renter ranges.", mtype='warning')
                    for i in range(4):
                        min_renters = i * 25
                        max_renters = min_renters + 25
                        print_info(f"Searching for markets with DSR {min_dsr}-{max_dsr} in {state} with renters between {min_renters} and {max_renters}", mtype='info')
                        ...
                        if len(segmented_data) >= 250:
                            print_info(f"Segmented data for DSR {min_dsr}-{max_dsr} in {state} with renters {min_renters}-{max_renters} still has 250+ results. Breaking down by individual DSR values.", mtype='warning')
                            ...
                            if len(segmented_data1) >= 250:
                                print_info(f"DSR {min_dsr} data still has 250+ results. Breaking down renter range {min_renters}-{max_renters} into smaller segments.", mtype='warning')
                                ...
                                for j in range(5):
                                    ...
                                    print_info(f"Searching DSR {min_dsr} in {state} with renters {segment_min}-{segment_max}", mtype='info')
                                ...
                                print_info(f"Found {len(segmented_data1)} markets for DSR {min_dsr} in {state} with renters {min_renters}-{max_renters}. Adding to all_state_data.", mtype='info')
                            ...
                            if len(segmented_data2) >= 250:
                                print_info(f"DSR {max_dsr} data still has 250+ results. Breaking down renter range {min_renters}-{max_renters} into smaller segments.", mtype='warning')
                                ...
                                for j in range(5):
                                    ...
                                    print_info(f"Searching DSR {max_dsr} in {state} with renters {segment_min}-{segment_max}", mtype='info')
                                    micro_segment_data, _ = get_data(max_dsr, max_dsr, state=state, min_renters=segment_min, max_renters=segment_max)
                                    all_state_data.extend(micro_segment_data)
                            else:
                                print_info(f"Found {len(segmented_data2)} markets for DSR {max_dsr} in {state} with renters {min_renters}-{max_renters}. Adding to all_state_data.", mtype='info')
                                all_state_data.extend(segmented_data2)                               
                            
                        else:
                            print_info(f"Found {len(segmented_data)} markets for DSR {min_dsr}-{max_dsr} in {state} with renters between {min_renters} and {max_renters}. Adding to all_state_data.", mtype='info')
                            all_state_data.extend(segmented_data)
                else:
                    print_info(f"Found {len(state_data)} markets for DSR {min_dsr}-{max_dsr} in {state}. Adding to all_state_data.", mtype='info')
                    all_state_data.extend(state_data)
        else:
            # Less than 250 results, use the original data
            all_state_data = data
            print_info(f"Found {len(data)} markets for DSR {min_dsr}-{max_dsr}. No segmentation needed.", mtype='info')
        ...
        if all_state_data:
            ...
            print_info(f"Saved {len(all_state_data)} records for DSR {min_dsr}-{max_dsr}. Total: {COUNT}", mtype='success')
        else:
            print_info(f"No data found for DSR {min_dsr}-{max_dsr}", mtype='warning')
        ...
        if min_dsr > 75:  # Example: stop at DSR 75
            print_info("Reached maximum DSR limit", mtype='info')
            break
    remove_duplicates(filename=filename)
    print_info(f"Data saved to {filename}", mtype='success')
